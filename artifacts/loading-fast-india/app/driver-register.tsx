...
export default function DriverRegisterScreen() {
  ...
  const [form, setForm] = useState({
    name: '', phone: '', email: '',
    aadhaarNumber: '', licenseNumber: '', licenseExpiry: '',
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      await login({ id, role: 'driver', name: form.name, phone: form.phone, email: form.email });
      ...
      <ScrollView style={styles.body} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        {step === 1 ? (
          <View>
            ...
            <Input label="Gmail / Email" placeholder="आपका email" value={form.email} onChangeText={(v) => { set('email', v); clearError('email'); }} keyboardType="email-address" autoCapitalize="none" error={errors.email} icon="mail" required />
            ...
            <Text style={[styles.rangeLabel, { color: colors.mutedForeground }]}>
              ट्रिप notification कितनी दूर तक मिले? (किमी में)
            </Text>
